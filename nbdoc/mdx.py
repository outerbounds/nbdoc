...
__all__ = ['InjectMeta', 'StripAnsi', 'InsertWarning', 'RmEmptyCode', 'MetaflowPreamble', 'UpdateTags',
    ...
    def preprocess_cell(self, cell, resources, index):
        if re.search('\s*python.+run.*', cell.source) and 'outputs' in cell:
            for o in cell.outputs:
                if o.name == 'stdout':
                    o['text'] = re.sub(self.pattern, r'\2', o.text).strip()
        return cell, resources
...
def get_mdx_exporter(template_file='ob.tpl'):
    ...
    pp = [InjectMeta, WriteTitle, CleanMagics, BashIdentify, MetaflowPreamble,
