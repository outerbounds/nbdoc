...
__all__ = ['InjectMeta', 'StripAnsi', 'InsertWarning', 'RmEmptyCode', 'MetaflowTruncate', 'UpdateTags',
           'MetaflowSelectSteps', 'WriteTitle', 'CleanMagics', 'BashIdentify', 'CleanShowDoc', 'MarkShowDoc',
           'ExecuteShowDocPreprocessor', 'get_mdx_exporter']
...
class MarkShowDoc(Preprocessor):
    """Mark notebooks that have ShowDoc cells."""
    def preprocess(self, nb, resources):
        for c in nb.cells:
            if _isShowDoc(c):
                resources['ShowDoc'] = True
        return nb, resources


class ExecuteShowDocPreprocessor(ExecutePreprocessor):
    "An `ExecutePreprocessor` that only executes `ShowDoc` and `import` cells"
    def preprocess_cell(self, cell, resources, index):
        if resources.get('ShowDoc') and not cell.metadata.get('script') and check_re_multi(cell, [_re_showdoc, _re_import]) :
...
def get_mdx_exporter(template_file='ob.tpl'):
    ...
    pp = [InjectMeta, WriteTitle, CleanMagics, BashIdentify, MetaflowTruncate,
          MetaflowSelectSteps, UpdateTags, InsertWarning, TagRemovePreprocessor, CleanShowDoc, RmEmptyCode, StripAnsi]
